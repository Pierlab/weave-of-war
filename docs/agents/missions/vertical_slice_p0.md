...
### Phase 7 progress
- 2026-01-03 — Ajout d'un panneau "Télémetrie" dans l'overlay debug : filtre OptionButton pour cibler un événement ou l'historique complet, rafraîchissement automatique via le nouveau signal `Telemetry.event_logged`, aperçu JSON compact des payloads, statut de persistance/session copiable dans le presse-papiers, et bouton d'actualisation manuel pour les sessions en pause.
...
- 2026-01-02 — AssistantAIAutoload archive désormais des traces de raisonnement pour les ordres, recommandations espionnage et alertes logistiques; la debug overlay affiche les trois flux et `docs/logs/assistant_ai_reasoning_sample_2026-01-02.jsonl` sert de log d'exemple pour l'analyse.
