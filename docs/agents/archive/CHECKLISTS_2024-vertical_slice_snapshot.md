# Archived Checklist — Vertical Slice P0 Snapshot (original 2024 plan)

> Archived on 2025-10-31 when the 2025 execution checklist replaced this document.
> Preserve for historical context only; do not update statuses here.
...
- [x] Documenter le mapping JSON → scènes/scripts dans le TDD afin que les agents sachent où charger et valider les données (référencer les scripts cibles sous `scripts/core` et `scripts/systems`).
...
- [x] Semaine 6 : Finaliser les compétences/points par tour et les formations d’unités (infanterie/archers/cavalerie avec postures), raccorder la télémétrie minimale (élan dépensé, résultats piliers, ruptures logistiques) et valider la boucle complète via tests automatisés + mise à jour des acceptances.
...
- [x] Documenter les décisions structurantes via des ADR (`docs/`), en suivant l’exemple fourni pour les doctrines sans cartes. (Livrable : [`docs/ADR_0002_event_bus_and_telemetry_autoloads.md`](docs/ADR_0002_event_bus_and_telemetry_autoloads.md).)
- [x] Maintenir un glossaire vivant (éventuellement dans `docs/`) pour les termes clés : Élan, Doctrine, Ordre, Zone logistique, Pilier, Ping, Posture, Inertie. (Livrable : [`docs/glossary.md`](docs/glossary.md).)
